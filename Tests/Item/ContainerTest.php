 ...
 * Load container item
 */
require_once realpath(dirname(__FILE__) . '/../../Formagic/Item/Container.php');
 ...
 * @version     $Id: ContainerTest.php 173 2012-05-16 13:19:22Z meweasle $
 **/
class Formagic_Item_Container_Test extends PHPUnit_Framework_TestCase
{
    ...
    public function testAddItem()
    {
        ...
        $result = $container->addItem('MockItem', $mockItemName);
        $this->assertSame($container, $result);

        $addedItem = $container->getItem($mockItemName);
        $this->assertInstanceOf('Formagic_Item_MockItem', $addedItem);
        $this->assertEquals($addedItem->getName(), $mockItemName);

        // create new object with "new" operator, then add
        $item = new Formagic_Item_MockItem($mockItemName2);
        $container->addItem($item);
        $addedItem = $container->getItem($mockItemName2);

        $this->assertInstanceOf('Formagic_Item_MockItem', $item);
    ...
    {
        Formagic::loadClass('Formagic_Item_MockItem');
        $container = new Formagic_Item_Container('test');
        $container->addItem('MockItem');
    }
    ...
    {
        $container = new Formagic_Item_Container('test');
        $container->addItem('MockItem', 'i1');
        $container->addItem('MockItem', 'i2');
        $container->addItem('MockItem', 'i3');
    ...
    public function testCountRecursive()
    {
        ...
        $container->addItem('MockItem', 'ci2');
        $container->addItem('MockItem', 'ci3');
    ...
    public function testGetItems()
    {
        ...
        $subContainer->addItem('MockItem', 'subItem');

        $expected = array(
            'i1' => new Formagic_Item_MockItem('i1'),
            'i2' => new Formagic_Item_MockItem('i2'),
            'i3' => new Formagic_Item_MockItem('i3'),
            'c1' => $subContainer
        );
    ...
    public function testGetItem()
    {
        ...
        $expected = new Formagic_Item_MockItem('testItem');
        $actual = $container->addItem($expected)->getItem('testItem');
        $this->assertSame($expected, $actual);

        // test deep level items
        $expected = new Formagic_Item_MockItem('subItem');
    ...
    public function testGetDeepLevelItemFail()
    {
        ...
        $container->addItem($subContainer);
        $actual = $container->getItem('itemDoesNotExist');
    }
    ...
    /**
     * Test setting values to container subitems
     */
    public function testSetValue()
    {
        ...
        // set subcontainer value
        $subItem = new Formagic_Item_MockItem('subItem');
        ...
        $expectedCoordiantes = array('x' => 1, 'y' => 1);
        $actualCoordiantes = $container->getItem('imgSubmit')->getClickCoordinates();
        $this->assertInternalType('integer', $actualCoordiantes['x']);
        $this->assertInternalType('integer', $actualCoordiantes['y']);
        $this->assertEquals($expectedCoordiantes, $actualCoordiantes);
        
        // test skipping clearing value
        $container = new Formagic_Item_Container('test');
        $testItem  = new Formagic_Item_MockItem('testMock', array(
            'value' => 0));
    ...
    public function testAddRule()
    {
        ...
        foreach($container as $item) {
            $this->assertTrue($item->hasRule('MockRule'));
        }
    }
    ...
    public function testAddFilter()
    {
        ...
        foreach($container as $item) {
            $this->assertTrue($item->hasFilter('MockFilter'));
        }
    }
    ...
    public function testDefaultRulesAndFilters()
    {
        ...
        $item = new Formagic_Item_MockItem('i2');
        $container->addItem($item);
        $this->assertTrue($item->hasRule('MockRule'));
        $this->assertTrue($item->hasFilter('MockFilter'));
    }
    ...
    public function testSetReadonly()
    {
        ...
        $container->addItem('MockItem', 'testMock');
        foreach($container as $item) {
            $html = $item->getHtml();
            $this->assertEquals(
                Formagic_Item_MockItem::HTML_OUTPUT_READONLY,
                $html
            );
        }
    }
    ...
    public function testGetValue()
    {
        ...
        // test subcontainer
        $subContainer = new Formagic_Item_Container('sub');
        $subContainer->addItem('MockItem', 'si1', array('value' => 'sv1'));
        $container->addItem($subContainer);
        $actual = $container->getValue();
    ...
    {
        $items = array(
            'i1' => new Formagic_Item_MockItem('i1', array('value' => 'v1')),
            'i2' => new Formagic_Item_MockItem('i2', array('value' => 'v2')),
            'i3' => new Formagic_Item_MockItem('i3', array('value' => 'v3'))
        );
        return $items;
    }
     ...
     */
    private function _getContainerWithItems($items = null)
    {
        if(!$items) {
